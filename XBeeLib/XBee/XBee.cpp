...
#define MAX_FRAME_PAYLOAD_LEN_SYNCR                                            \
  (1 /* type */ + 1 /* id */ + 2 /* at cmd*/ + 1 /* status */ +                \
   2 /* MY sender */ + 8 /* 64b sender */ + 20 /* max id */ +                  \
   1 /* null ter */ + 2 /* MY parent */ + 1 /* dev type */ +                   \
   1 /* source event */ + 2 /* prof. id */ + 2 /* man. id */)

FrameBuffer XBee::_framebuf_app(FRAME_BUFFER_SIZE, MAX_FRAME_PAYLOAD_LEN);
FrameBuffer XBee::_framebuf_syncr(FRAME_BUFFER_SIZE_SYNCR,
                                  MAX_FRAME_PAYLOAD_LEN_SYNCR);
...
bool XBee::check_radio_flow_control() {
  ...
  if (_serial_flow_type == SerialBase::RTSCTS ||
      _serial_flow_type == SerialBase::CTS) {
    cmdresp = get_param("D7", &value);
    if (cmdresp != AtCmdFrame::AtCmdRespOk) {
      // digi_log(LogLevelError, "Could not read CTS configuration. Error
      // %d\r\n", cmdresp);
      return false;
    } else if (value != 1) {
      // digi_log(LogLevelError, "Bad CTS configuration. Radio 'D7' param is %d
      // and should be 1\r\n", value);
      return false;
    }
  }
  ...
  if (_serial_flow_type == SerialBase::RTSCTS ||
    ...
    if (cmdresp != AtCmdFrame::AtCmdRespOk) {
      // digi_log(LogLevelError, "Could not read RTS configuration. Error
      // %d\r\n", cmdresp);
      return false;
    } else if (value != 1) {
      // digi_log(LogLevelError, "Bad RTS configuration. Radio 'D6' param is %d
      // and should be 1\r\n", value);
      return false;
    }
  }
...
/* Class constructor */
XBee::XBee(PinName tx, PinName rx, PinName reset, PinName rts, PinName cts,
           int baud)
    : _mode(ModeUnknown), _hw_version(0), _fw_version(0),
      _timeout_ms(SYNC_OPS_TIMEOUT_MS), _dev_addr64(ADDR64_UNASSIGNED),
      _reset(NULL), _tx_options(0), _hw_reset_cnt(0), _wd_reset_cnt(0),
      _modem_status_handler(NULL), _modem_status(AtCmdFrame::HwReset),
      _initializing(true), _node_by_ni_frame_id(0) {
...
RadioStatus XBee::init(void) {
  ...
  if (_modem_status_handler != NULL) {
    const ApiFrame frame =
        ApiFrame(ApiFrame::AtModemStatus, (uint8_t *)&_modem_status,
                 sizeof(_modem_status));
...
uint64_t XBee::get_addr64() const { return _dev_addr64; }
...
RadioStatus XBee::wait_for_module_to_reset(volatile uint16_t *rst_cnt_p,
                                           uint16_t init_rst_cnt) {
  Timer timer = Timer();
  timer.start();

  int16_t t =
      chrono::duration_cast<chrono::milliseconds>(timer.elapsed_time()).count();
  while (t < RESET_TIMEOUT_MS) {
    thread_sleep_for(100);
    t = chrono::duration_cast<chrono::milliseconds>(timer.elapsed_time())
            .count();
  }
  timer.stop();
  timer.reset();
...
void XBee::uart_read_cb(void) {
  ...
  case WAITING_FOR_LENGTH_LSB:
    ...
    if (framelen > MAX_FRAME_PAYLOAD_LEN) {
      ...
      // digi_log(LogLevelWarning, "Frame dropped, frame too long. Increase
      // MAX_FRAME_PAYLOAD_LEN define\r\n");
...
#define CACHED_SIZE 3
    ...
    if (framelen <= CACHED_SIZE) {
      if (!bytes_read) {
        ...
        if (framebuf == NULL) {
          // digi_log(LogLevelWarning, "Discarding not supported frame type
          // %02x\r\n", frame_type);
          rxstate = WAITING_FOR_START_FRAME;
        } else {
          frame = framebuf->get_next_free_frame();
          if (frame == NULL) {
            /* It's not possible to achive this condition as we discard older
             * frames and only one frame can be used by syncr. commands */
            assert(frame != NULL);
            rxstate = WAITING_FOR_START_FRAME;
          } else {
    ...
    if (bytes_read < CACHED_SIZE) {
      ...
      const ApiFrame::ApiFrameType frame_type =
          (ApiFrame::ApiFrameType)frame_cached[0];
      switch (frame_type) {
      case ApiFrame::RemoteCmdResp:
      case ApiFrame::TxStatusZBDM:
      case ApiFrame::TxStatus:
      ...
      case ApiFrame::AtCmdResp:
        if ((frame_cached[1] != _node_by_ni_frame_id) &&
            (frame_cached[2] == 'N') && (data == 'D')) {
          framebuf = &_framebuf_app;
        } else {
          framebuf = &_framebuf_syncr;
        }
  ...
  case WAITING_FOR_CHECKSUM:
    ...
    if (chksum == 0xFF) {
       ...
       * the library. The frame is also queued to allow processing it other
       * handlers registered. Note that radio_status_update() has to be fast to
       * minimize the impact of processing the funcion here */
      if (frame->get_frame_type() == ApiFrame::AtModemStatus) {
        ...
        framebuf->complete_frame(frame);
        /* Note, the frame will be released elsewhere, once it has been
         * processed */
      }
    } else {
      framebuf->free_frame(frame);
...
/* This is a pure virtual function, but exists here because its called from this
 * class to to update the status of the object, and can be called before the
 * construction of the object has been completed and the virtual functions
 * filled */
void XBee::radio_status_update(AtCmdFrame::ModemStatus modem_status) {
  UNUSED_PARAMETER(modem_status);
}

void XBee::set_timeout(uint16_t timeout_ms) { this->_timeout_ms = timeout_ms; }

uint16_t XBee::get_timeout(void) const { return _timeout_ms; }
...
void XBee::send_byte_escaping_if(uint8_t data) { _uart->write(&data, 1); }
void XBee::send_uart_data_set() {
  // 7E 00 09 08 01 49 44 30 31 32 33 34 6F
  uint8_t t[] = {0x7E, 0x00, 0x09, 0x08, 0x01, 0x49, 0x44,
                 0x30, 0x31, 0x32, 0x33, 0x34, 0x6F};
  _uart->write(&t, sizeof(t));
}
void XBee::send_uart_data(void) {

  uint8_t t[] = {0x7E, 0x00, 0x04, 0x08, 0x01, 0x49, 0x44, 0x69};
  _uart->write(&t, sizeof(t));
}

void XBee::send_api_frame(ApiFrame *frame) // Refactored
{
  ...
  uint8_t data[frame->get_data_len() + 4];
  /* Start Frame*/
  data[0] = (uint8_t)DR_START_OF_FRAME;
  /* Now the length */
  ...
  data[3] = frame->get_frame_type();
  for (int i = 0; i < frame_len - 1; i++) {
    data[i + 4] = frame->get_data()[i];
    chksum += frame->get_data()[i];
  }
  /* And finally send the checksum */
  data[sizeof(data)] = ~chksum;
...
XBee::RadioProtocol XBee::get_radio_protocol(void) const {
  ...
  if (hw_version_msb == XB24_AXX_XX || hw_version_msb == XBP24_AXX_XX) {
...
#endif
  } else if (hw_version_msb == XB24_BXIX_XXX ||
             hw_version_msb == XBP24_BXIX_XXX) {
    if (fw_4_bytes_len &&
        ((fw_nibble_3 == 1 && fw_nibble_1 == 2 && fw_nibble_0 == 0) ||
         fw_nibble_3 == 2)) {
      return ZigBee;
    }
#ifdef EXTRA_XBEE_PROTOCOLS
...
#endif
#ifdef EXTRA_XBEE_PROTOCOLS
  } else if (hw_version_msb == XB24_WF || hw_version_msb == WIFI_ATHEROS ||
             hw_version_msb == SMT_WIFI_ATHEROS) {
    return XBeeWiFi;
#endif
    ...
    return DigiPoint;
  } else if (hw_version_msb == XBP24C_TH_DIP ||
             hw_version_msb == XB24C_TH_DIP ||
             hw_version_msb == XBP24C_S2C_SMT) {
...
TxStatus XBee::send_data(ApiFrame *frame) {
  ...
  resp_frame = get_this_api_frame(frame->get_frame_id(), ApiFrame::TxStatusZBDM,
                                  ApiFrame::TxStatus);
  if (resp_frame == NULL) {
    return resp;
  }
