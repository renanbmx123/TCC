...
uint8_t data[DATA_SIZE],maxPowerLevel = 4, TimeoutFactor = 1; // char data[200],  /*Data to send*/
uint8_t sensor_change = 0;
uint8_t sendDataSize = 0;
uint16_t id = 0, year = 0;
uint32_t t_init, t_con, t_read, t_send, t_sleep;
...
void state_machine() {
  ...
  while (1) {
    ...
    case ST_INIT:
      ...
      t.stop();
      //led1 = 0;
      t_init = chrono::duration_cast<chrono::milliseconds>(t.elapsed_time()).count();
    ...
    case ST_CON:
      ...
      st_con();
      t.stop();
      t_con=chrono::duration_cast<chrono::milliseconds>(t.elapsed_time()).count();
      //led2 = 0;
      //printf("CON, t:%llu\n",chrono::duration_cast<chrono::milliseconds>(t.elapsed_time()).count());
      t.reset();
      break;
    case ST_RDATA:
      ...
      st_rdata();
      t.stop();
      t_read=chrono::duration_cast<chrono::milliseconds>(t.elapsed_time()).count();
      //led3 = 0;
      // printf("RDATA, t:%llu\n",chrono::duration_cast<chrono::milliseconds>(t.elapsed_time()).count());
      t.reset();
      break;
    case ST_SDATA:
      ...
      st_sdata();
      t.stop();
      t_send=chrono::duration_cast<chrono::milliseconds>(t.elapsed_time()).count();
      //led4 = 0;
      //printf("SDATA, t:%llu\n",chrono::duration_cast<chrono::milliseconds>(t.elapsed_time()).count());
      t.reset();
      break;
    case ST_SLEEP:
      ...
      st_sleep();
      t.stop();
      t_sleep=chrono::duration_cast<chrono::milliseconds>(t.elapsed_time()).count();
      //printf("SLEEP, t:%llu\n", chrono::duration_cast<chrono::milliseconds>(t.elapsed_time()).count());
      t.reset();
      break;
    }
...
void st_rdata() {
  ...
  sendDataSize = sprintf(
      ...
      Balt, tgps.altitude.meters(), vbat, xbee_power_level,t_init, t_con, t_read, t_send, t_sleep);

#if defined(WRITE_LOCAL_DATA)
  file_write((char *)data);
#endif
...
{
  if (_sleep_req == NULL)
    _sleep_req = new DigitalOut(p22); //   Xbee on sleep pin.; //Xbee sleep request pin.
  //printf("wakeup_radio\n");
  //assert(_sleep_req != NULL);
  //_sleep_req.write(0);
  _sleep_req->write(0) ;
...
void file_write(char *data_file) {
  auto dummy = fopen("/local/out.csv", "a");
  sm.fp = dummy;
  if (sm.fp == NULL) {
    sm.fp = fopen("/local/out.csv", "w");
    // if(sm.fp == NULL){
